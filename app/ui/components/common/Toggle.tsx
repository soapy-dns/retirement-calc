    ...
    <div className="grid w-1/3 grid-cols-2">
      {options.map((option, index) => {
        ...
        return (
          <div
            key={`${optionValue}`}
            className={`flex justify-center border-primary border  py-1  first:rounded-l-md last:rounded-r-md ${
          ...
          >
            ...
            <input
              ...
              value={optionValue}
              onClick={onChange}
              className="opacity-0 focus:outline-none focus:ring-0 focus:border-red focus:border-2 "
            />
          </div>
        )
      })}
